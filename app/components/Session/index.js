...
import { Text, TouchableOpacity, Image, FlatList, Linking, Platform } from "react-native";
...
function TutorialModal(props) {
	...
	const style = {
		...
		logo: { height: 80, width: 80, borderRadius: 20 },
		instruction: {
			fontWeight: "500",
			fontSize: 14,
			color: "#000"
	...
	const GoToYatuViewerStore = () => {
		const url = Platform.select({
			ios: "https://play.google.com/store/apps/details?id=com.yatudashboard.viewer&hl=en&gl=US",
			android: "https://play.google.com/store/apps/details?id=com.yatudashboard.viewer&hl=en&gl=US"
		})
		...
		<BaseIIModal {...props}>
			<VStack py={3} space={3} alignItems={"center"}>
				...
				<VStack space={2} alignItems={"center"}>
					<View width={"90%"}>
						<Text style={style.description}>To get started on session-based viewer, install</Text>
					</View>
					<TouchableOpacity onPress={GoToYatuViewerStore}>
						<VStack space={1} alignItems={"center"}>
							<Text style={[style.instruction, { color: "#6334D3" }]}>Yatu Viewer</Text>
							<Image source={Images.YatuViewerLogo} resizeMode={"contain"} style={style.logo} />
						</VStack>
					</TouchableOpacity>
				</VStack>
				...
				<VStack p={3} space={3} width={"90%"}
					...
					<VStack space={2}>
						...
						<Text style={style.instruction}>1. Yatu Viewer is a session-based viewer app, to allow another user to help monitor your iOT devices. </Text>
						<Text style={style.instruction}>2. Each subscribed users have </Text>
						<View alignItems={"center"}>
							<Text style={{ fontSize: 18, fontWeight: "500", color: "#6334D3" }}>5 viewer</Text>
						</View>
						<View alignItems={"center"}>
							<Text style={[style.instruction, { fontStyle: 'italic' }]}>make sure to use wisely!</Text>
						</View>
					</VStack>
				</VStack>
